...
| `agent_server.py` | Clinical-triage **AI agent** (FastAPI, `:8000`). Runs a real tool-calling loop over a pluggable backend (**local Ollama by default**, Claude optional) with 6 tools — historical baseline, cross-session memory, vitals reference, EWMA trend, a medical-model consult, and a clinician-notify action — plus a deterministic safety floor and FHIR R4 Observation. Persists memory to `./agent_memory` + `./fhir_store`. See [`docs/agent-design.md`](./docs/agent-design.md). |
...
# for the AI triage agent (agent_server.py) — local Ollama backend by default:
pip install fastapi "uvicorn[standard]" ollama anthropic deltalake pandas
# then: ollama serve && ollama pull llama3.1:8b && ollama pull meditron
# (Claude backend instead: AGENT_BACKEND=anthropic + export ANTHROPIC_API_KEY=...)
```
...
- **AI agent layer**: `agent_server.py` is a local-first, provider-portable agent with a real
  tool-calling loop — Ollama by default (free, local), Claude optional behind the same interface.
  Phases 1 & 4 are built locally: 6 tools, cross-session memory, EWMA trend detection, a
  clinician-notify action, a deterministic safety floor, and FHIR output; the Claude backend
  (compare/fallback) is deferred. Full design + roadmap: [`docs/agent-design.md`](./docs/agent-design.md).
